...
resource "null_resource" "crds" {
  ...
  provisioner "local-exec" {
    command = "kubectl ${length(self.triggers.kubeconfig) > 0 ? "--kubeconfig" : ""} ${self.triggers.kubeconfig} apply -f - <<'EOF'\n${data.template_file.crds.rendered}\nEOF"
  }

  provisioner "local-exec" {
    when    = destroy
    command = "kubectl ${length(self.triggers.kubeconfig) > 0 ? "--kubeconfig" : ""} ${self.triggers.kubeconfig} delete -f ${path.module}/templates/crds.yaml"
  }
}
